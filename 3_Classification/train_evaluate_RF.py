...
from sklearn.metrics import classification_report, confusion_matrix, accuracy_score, precision_recall_curve, PrecisionRecallDisplay
...
## find optimal number of trees
    ...
    for n in range(1,101):  # loop over number of trees between 1 and 100
    ...
    ntree = input('Optimal number of trees in the forest: ')   # enter in the terminal the number of trees you want to use for further analysis
    ntree = int(ntree)
    print(ntree)
    ntree = 150
    ...
    if df.columns.str.contains('target').any():
            df = df.drop(['target'], axis = 1)
    else:
        df = df    
    if df.columns.str.contains('ROI_img_name').any():
            df = df.drop(['ROI_img_name'], axis = 1)
    else:
        df = df    
    if df.columns.str.contains('well').any():
            df = df.drop(['well'], axis = 1)
    else:
        df = df    
    if df.columns.str.contains('true_density').any():
            df = df.drop(['true_density'], axis = 1)
    else:
        df = df    
